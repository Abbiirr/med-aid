...
function Index() {
  ...
  const getRemainingTime = async (councilHourID) => {
    try {
  ...
  const customList = (items) => (
    <Paper sx={{ width: 200, overflow: "auto" }}>
      {items == left ? (
        <Alert severity="info"> Patients Waiting</Alert>
      ) : (
        <Alert severity="success">Checkup Done </Alert>
      )}
          ...
          return (
            <div>
              ...
              >
                ...
                <ListItemText id={labelId} primary={`Patient ${value + 1}`} />
              </ListItem>
            </div>
          );
        })}
